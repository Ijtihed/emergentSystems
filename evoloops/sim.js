(() => {
    ...
    // Evoloops (Sayama 1999): Langton-style loops with mutation during replication
    // 9 states (0-8), von Neumann neighborhood, modified transition rules
    // Key difference from Langton: some transitions introduce errors (mutations)
    const palette = [
        [8, 8, 8],       // 0: empty
        [40, 80, 160],    // 1: core
        [200, 60, 60],    // 2: sheath
        ...
        [200, 60, 200],   // 5: signal
        [60, 200, 200],   // 6: signal
        [200, 200, 200],  // 7: signal
        [120, 80, 40]     // 8: dissolving
    ];
    ...
    // Evoloop transition rules: based on Langton's rules with mutation modifications
    // When a collision or copying error occurs, state 8 (dissolving) appears
    // which causes partial dissolution and allows mutant loops to form
    const rules = new Map();

    // Base Langton rules (same as langton-loops) plus evoloop modifications
    const ruleStrings = [
        ...
        '12224:4','12227:7','12243:4','12254:7','12324:4','12327:7','12425:5',
        '12527:5','12622:2',
        ...
        // Evoloop additions: collision rules that produce state 8 (dissolving)
        '80002:0','80012:0','80022:0','80032:0','80052:0','80062:0','80072:0',
        '80102:0','80112:0','80122:0','80202:0','80212:0','80222:0','80272:0',
        '80522:0','80722:0',
        // Any cell next to 8 that is sheath (2) dissolves
        '20082:8','20028:8','20208:8','20802:8','20008:8',
        '02008:8','00208:8','00028:8','00082:8','00008:0',
        // State 8 decays to 0
        '08000:0','00800:0','00080:0','00008:0','80000:0',
        '88888:0','80808:0','08080:0'
    ];
    ...
    function placeLoop(ox, oy) {
        for (let r = 0; r < loopPattern.length; r++) {
            ...
            for (let c = 0; c < row.length; c++) {
                const x = ox + c, y = oy + r;
                if (x >= 0 && x < W && y >= 0 && y < H) grid[y * W + x] = +row[c];
            }
        }
    }
    ...
    function init() {
        grid.fill(0);
        totalSteps = 0;
        // Place initial loops at different positions to encourage collisions
        placeLoop((W / 2 - 4) | 0, (H / 2 - 4) | 0);
        placeLoop((W / 4 - 4) | 0, (H / 4 - 4) | 0);
        placeLoop((3 * W / 4 - 4) | 0, (3 * H / 4 - 4) | 0);
    }
    ...
    function loop() {
        ...
        render();
        let pop = 0;
        for (let i = 0; i < W * H; i++) if (grid[i] > 0) pop++;
        statusEl.textContent = 'step ' + totalSteps + ', ' + pop + ' active cells';
